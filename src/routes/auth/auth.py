...
from ..config import db_user_service, auth_bp
...
def login() -> str:
    if request.method == "POST":
        if not (
            ...
            and db_user_service.authenticate_user(
...
def register() -> str:
    if request.method == "POST":
        if db_user_service.user_exists(request.form["email"]):
            return render_template("auth/register.html", register_error=True)

        db_user_service.create_user(
